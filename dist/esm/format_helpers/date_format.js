 ...
 */
export function dateFormatAgo(value) {
  var date = new Date(value); // If date is not a valid Date object, return an empty string
  ...
  oneMonthAgo.setMonth(now.getMonth() - 1); // Less than a month
...
export function dateTimeFormatAgo(value) {
  ...
  var now = new Date(); // Check browser compatibility
  // If no available, return the date formated
  ...
  var deltaSeconds = Math.max(Math.round((now.getTime() - date.getTime()) / 1000), 0); // Less than a minute

  if (deltaSeconds <= 60) {
    return relativeFormatter.format(-deltaSeconds, 'second');
  }

  var deltaMinutes = Math.round(deltaSeconds / 60); // Less than an hour

  if (deltaMinutes < 60) {
    return relativeFormatter.format(-deltaMinutes, 'minute');
  }

  var deltaHours = Math.round(deltaMinutes / 60); // Less than a day
  ...
  var deltaDays = Math.round(deltaHours / 24); // Less than a month

  if (date > oneMonthAgo) {
    var output = relativeFormatter.format(-deltaDays, 'day'); // Is yesterday or displayTime is set to true : display time
